...
public class FluentExplicitWait7 {
	
	public static WebElement fluentWaitToXpathElem (WebDriver driver, String xapthStr, int timeout, int polling) {		
		FluentWait wait = new FluentWait(driver);
		wait.withTimeout(Duration.ofSeconds(timeout));
		wait.pollingEvery(Duration.ofMillis(polling));
		wait.ignoring(NoSuchElementException.class);	
		return (WebElement) wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xapthStr)));		
	}
	...
	public static void main(String[] args) throws InterruptedException {
		...
		WebElement foo= fluentWaitToXpathElem(driver, "//div[@id='finish']/h4",7,250);
